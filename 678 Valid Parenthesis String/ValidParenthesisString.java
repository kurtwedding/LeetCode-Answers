public class ValidParenthesisString {
    ...
    public static boolean checkValidString(String s){
        ...
        for(int i = s.length()-1; i >= 0; i--){
            if(s.charAt(i) == ')') tempA++; // if there is a ) add to the count
            else if(s.charAt(i) == '(') tempB++; // if there is a ( take 1 from the count
            // if the count is 0, there is an equal number of ('s and )'s
            else if(s.charAt(i) == '*') tempStar++; // this can be counted as a ( or )
            if(tempA + tempStar < tempB) return false; // if at any time the possible number of open brackets is less than the closing ones, return false
        }
